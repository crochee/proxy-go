...
func generateOperationName(prefix string, parts []string, sep string, spanLimit int) string {
	...
	if spanLimit > 0 && len(name) > spanLimit {
		if spanLimit < maxLength {
			logger.Warnf("SpanNameLimit cannot be lesser than %d: falling back on %d, maxLength, maxLength+3",
				maxLength)
...
func computeHash(name string) string {
	...
	if _, err := hash.Write(data); err != nil {
		// Impossible case
		logger.Errorf("Failed to create Span name hash for %s: %v", name, err)
	}
